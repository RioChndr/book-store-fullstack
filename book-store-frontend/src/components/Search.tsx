...
const DatabasePage: Result[] = [
  {url: "/dashhoard", title: "Dashboard", pageTitle: "Dashboard"},
  {url: "/dashboard/users", title: "User List", pageTitle: "User List"},
  {url: "/dashboard/sites", title: "Sites List", pageTitle: "Site List"},
  {url: "/dashboard/posts", title: "Posts List", pageTitle: "Posts List"},
]

function DatabasePageSearch(query: string, props: {
  limit:number,
}){
  return DatabasePage.filter(data => data.title.toLowerCase().includes(query.toLowerCase()))
}
        ...
        return [
          {
            ...
            getItems() {
              return DatabasePageSearch(query, { limit: 5 })
            },
            getItemUrl({ item }) {
              return item.url
            },
